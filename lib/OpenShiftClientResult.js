...
const logger = { 
    info: debug('info:OpenShiftClientResult'),
    trace: debug('trace:OpenShiftClientResult'),
};
...
module.exports = class OpenShiftClientResult {
  ...
  _buildStdOutObject(stdOutHeaders, stdOutValues) {
    logger.trace(`_buildStdOutObject - stdOutHeaders: ${stdOutHeaders}`)
    logger.trace(`_buildStdOutObject - stdOutValues: ${stdOutValues}`)
  ...
  _parseProcessStdOutDataIntoObject(stdOutData) {
    ...
    logger.trace(`_parseProcessStdOutDataIntoObject - stdout: ${stdOut}`)

    const stdOutRows = this._getStdOutRows(stdOut);

    logger.trace(`_parseProcessStdOutDataIntoObject - stdOutRows: ${stdOutRows}`)
    ...
    if (!this.stdOutHeaders.length) {
      ...
      logger.trace(`_parseProcessStdOutDataIntoObject - set stdOutHeaders: ${this.stdOutHeaders}`)
  ...
  _parseProcessStdOutDataIntoObject2(stdOutData) {
    let stdOut = stdOutData.toString();

    logger.trace(`_parseProcessStdOutDataIntoObject2 - stdout: ${stdOut}`)

    const stdOutRows = this._getStdOutRows(stdOut);

    logger.trace(`_parseProcessStdOutDataIntoObject2 - stdOutRows: ${stdOutRows}`)

    if (!stdOutRows.length) {
      return {};
    }

    this.stdOutHeaders = ['name', 'desired', 'current'];
    this.stdOutValues = this._parseValueStdOutString(stdOutRows[stdOutRows.length-1]);
  ...
  _isDeploymentComplete(stdOutObject) {
    ...
    if (!stdOutObject || !stdOutObject.desired || !stdOutObject.current) {
      logger.trace(`_isDeploymentComplete - false`)
      return false;
    }

    if (stdOutObject.desired === stdOutObject.current) {
      logger.trace(`_isDeploymentComplete - true`)
      return true;
    }
   ...
   */
  waitForDeployment() {
    logger.info('waitForDeployment - start')
    ...
    logger.info('waitForDeployment - wait for stdout')

    this.proc.stdout.on('data', data => {
      // Kill the child process if the current # of replicas is equal to the desired # of replicas (job is complete)
      const stdOutObject = this._parseProcessStdOutDataIntoObject2(data);

      logger.trace(`waitForDeployment - stdOutObject: ${JSON.stringify(stdOutObject)}`)

      if (this._isDeploymentComplete(stdOutObject)) {
