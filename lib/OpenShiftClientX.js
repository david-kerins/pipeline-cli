...
const { isArray } = Array;
const info = require('debug')('info:OpenShiftClientX');
const trace = require('debug')('trace:OpenShiftClientX');
...
class OpenShiftClientX extends OpenShiftClient {
  ...
  applyRecommendedLabels(resources, appName, envName, envId, instance) {
    ...
    const envLabels = {
      'env-name': envName,
      'env-id': envId,
      //ddk 'github-repo': this.git.repository,
  ...
  copyRecommendedLabels(source, target) {
    //ddk ['app', 'app-name', 'env-name', 'env-id', 'github-repo', 'github-owner'].forEach(labelName => {
    ['app', 'app-name', 'env-name', 'env-id', 'github-owner'].forEach(labelName => {
  ...
  */
  ...
  async applyAndDeploy(resources, appName) {
    ...
    const watchDeploymentProcess = this.rawAsync('get', 'dc', {
      selector: `app=${appName}`,
      output: `custom-columns=:.metadata.name,:.status.replicas,:.status.readyReplicas`,
      watch: 'true'
    });

    return new Promise(resolve => {
      if (existingDC.stdout !== newDCs.stdout) {
        logger.trace('applyAndDeploy - 1')

        const openShiftClientResult = new OpenShiftClientResult(watchDeploymentProcess);
        openShiftClientResult.waitForDeployment();
      } else {
        logger.trace('applyAndDeploy - 2')

        watchDeploymentProcess.kill('SIGTERM');
      }
      watchDeploymentProcess.on('exit', (code) => {
        logger.trace('applyAndDeploy - 3 - code: ', code)
